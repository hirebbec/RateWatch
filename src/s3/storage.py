...
class S3Storage:
    ...
    async def get_file(self, full_key: str) -> :
        key = full_key.removeprefix(self._s3_bucket_name)

        try:
            file = await self._s3_client.get_object(Bucket=self._s3_bucket_name, Key=key)
    ...
    async def upload_file(self, full_key: str, data: bytes, content_type: str | None = None) -> bool:
        ...
        try:
            await self._s3_client.put_object(Bucket=self._s3_bucket_name, Key=key, Body=data, ContentType=content_type)
            return True
